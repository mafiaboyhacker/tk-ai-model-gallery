...
// import { uploadToRailwayVolume, generateVideoThumbnail } from './railway-storage';
...
async function processSingleFile(
  ...
  if (metadata.fileType === FileType.VIDEO) {
    ...
    const videoResult = { url: `/uploads/${file.filename}` }; // 임시 구현
    // const thumbnailResult = await generateVideoThumbnail(file.buffer, file.filename);
    const thumbnailResult = { url: `/thumbs/${file.filename}.jpg` }; // 임시 구현
    ...
    //   ],
    // });
    const imageResult = { url: `/uploads/${file.filename}`, thumbnails: [{ url: `/thumbs/${file.filename}` }] }; // 임시 구현

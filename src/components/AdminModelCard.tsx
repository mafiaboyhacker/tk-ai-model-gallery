...
export default function AdminModelCard({
  ...
  const resolvedVideoSource = useMediaObjectUrl({
    id,
    originalUrl,
    enabled: type === 'video',
    variant: 'original'
  ...
  const resolvedModalImageSource = useMediaObjectUrl({
    id,
    originalUrl,
    enabled: isModalOpen && type !== 'video',
    variant: 'original'
  })

  const videoSource = useMemo(
    () => resolvedVideoSource || originalUrl || normalizedImageUrl,
    [normalizedImageUrl, originalUrl, resolvedVideoSource]
  )
        ...
        >
          <div className="relative max-w-4xl max-h-full">
            {type === 'video' ? (
              ...
              <OptimizedImage
                src={resolvedModalImageSource || originalUrl || normalizedImageUrl}
                alt={`${imageAlt} (고화질 원본)`}
                width={width}
                height={height}
                className="max-w-full max-h-full object-contain"

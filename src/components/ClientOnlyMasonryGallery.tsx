...
const ClientOnlyMasonryGallery = memo(function ClientOnlyMasonryGallery({ models, loading = false }: MasonryGalleryProps) {
  ...
  // 🚀 Masonic hooks integration with guaranteed valid containerRef
  const { offset, width } = useContainerPosition(
    mounted && containerRef.current ? containerRef : { current: document.documentElement },
  ...
  // 🛡️ FINAL FIX: Use offset directly from useContainerPosition (Context7 confirmed)
  const scrollerTarget = offset
  ...
  try {
    ...
    // Only catch result processing errors, not Hook call errors
    if (process.env.NODE_ENV === 'development') {
      console.warn('🚨 Scroller result processing error:', error)
    }
    scrollTop = 0
    isScrolling = false
  }
  ...
  // 🛡️ Always call Hooks - never conditional

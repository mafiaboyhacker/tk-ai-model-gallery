...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const file of files) {
      ...
      // Railway Volume에 파일 업로드 (임시 비활성화)
      // const uploadResult = await uploadToRailwayVolume(processedFile, `models/${Date.now()}-${filename}`)
      const uploadResult = { url: `/temp/${filename}`, key: `temp-${Date.now()}` }

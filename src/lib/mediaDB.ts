...
class MediaDB {
  ...
  async getMediaObjectUrl(id: string, variant: 'original' | 'thumbnail' = 'original'): Promise<string | null> {
    ...
    if (variant === 'original') {
      if (media.originalBlob) {
        return URL.createObjectURL(media.originalBlob)
      }
      ...
      if (media.thumbnailBlob) {
        return URL.createObjectURL(media.thumbnailBlob)
      }
